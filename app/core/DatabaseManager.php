...
class DatabaseManager {
	...
	public static function init($exceptionMode = true) {
		...
		if (isset(\Europa\Core\Kernel::$DATABASE['port']) && \Europa\Core\Kernel::$DATABASE['port'] != null) {
			$dsn = "mysql:dbname=".\Europa\Core\Kernel::$DATABASE['database'].";host=".\Europa\Core\Kernel::$DATABASE['host'].";port=".$db['port'];
		} else {
	...
	public static function SQL($command, $arguments = null, $autoFetch = false) {
		if (!self::$IS_INIT) {
			self::init();
		}
		...
		if (!$autoFetch) {
			return array($exec,$req, $req->errorInfo(),self::$INSTANCE->lastInsertId());
		}
		$f = self::fetchSQL(array($exec,$req));
		return array($exec,$req, $req->errorInfo(),self::$INSTANCE->lastInsertId(), $f);
